...
if not st.session_state.id_token:
    ...
    st.markdown("---")
    st.markdown("*Note: Google Sign-in can be added using a custom Streamlit component, but this demo uses Firebase Identity Toolkit for Email/Password.*")
    ...
    search_query = st.text_input("🔍 Search tasks by title or description...", "")
    ...
    if todos:
        # Filter logic
        if search_query:
            filtered_todos = [t for t in todos if search_query.lower() in t.get("title", "").lower() or search_query.lower() in t.get("description", "").lower()]
        else:
